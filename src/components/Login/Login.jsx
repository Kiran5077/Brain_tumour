...
const Login = ({ role = "user" }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`https://brain-tumourbackend.onrender.com${loginEndpoint}`, {
        method: "POST",
        headers: {
  ...
  return (
    <div className="relative min-h-screen bg-cover top-5 bg-center text-white font-nunito" style={{ backgroundImage: `url('/assets/brainn.webp')` }}>
      {/* Overlay dark blur */}
      ...
      >
        <section className="text-center py-14 px-4 sm:px-10 md:px-20 text-white">
          ...
          <p className="text-green-300 text-sm">
            Home / <span className="text-green-200">{role === "admin" ? "Admin" : "User"} Login</span>
          </p>
        </section>
      </motion.div>
      ...
      <main className="relative z-10 w-full max-w-md mx-auto mt-4 px-4 sm:px-6 lg:px-8">
        ...
        >
          <h3 className="text-center text-2xl font-semibold mb-6 text-white">Login to your account</h3>
          <form className="space-y-5" onSubmit={handleSubmit}>
            ...
            {message && (
              <p className={`mt-4 text-center text-sm ${message.toLowerCase().includes("success") ? "text-green-300" : "text-red-300"}`}>
                {message}
              </p>
            )}
          </form>
